        ...
        <div className="flex items-center justify-between gap-8">
          ...
          <div className="flex items-center gap-10">
            <Link to="/" className="flex items-center gap-2.5 group flex-shrink-0">
              <div className="w-9 h-9 bg-navy-600 rounded-xl flex items-center justify-center shadow-lg shadow-navy-600/20 group-hover:scale-105 transition-transform">
                <span className="text-white font-bold text-lg font-heading">M</span>
              </div>
              <span className={`text-xl font-bold font-heading tracking-tight transition-colors ${
                showWhiteBg || isHome ? 'text-slate-900' : 'text-white'
              }`}>
                Mile<span className="text-navy-600">stone</span>
              </span>
            </Link>

            <nav className="hidden lg:flex items-center gap-7">
              <Link to="/jobs" className={`text-[11px] font-bold uppercase tracking-[0.1em] transition-colors ${
                showWhiteBg || isHome ? 'text-slate-500 hover:text-navy-600' : 'text-white/70 hover:text-white'
              }`}>Find Work</Link>
              <Link to="/blogs" className={`text-[11px] font-bold uppercase tracking-[0.1em] transition-colors ${
                showWhiteBg || isHome ? 'text-slate-500 hover:text-navy-600' : 'text-white/70 hover:text-white'
              }`}>Resources</Link>
            </nav>
          </div>
              ...
              {searchBar ? (
                ...
                <div className="relative w-full group/search">
                  <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                    <i className="fas fa-search text-[10px] text-slate-400 group-focus-within/search:text-navy-600 transition-colors"></i>
                  </div>
                  <form onSubmit={handleSearch} className="relative">
                    <input 
                      ...
                      className={`w-full pl-10 pr-4 py-2 rounded-full text-xs font-medium outline-none transition-all ${
                        showWhiteBg 
                          ? 'bg-slate-100/50 border border-slate-100 focus:bg-white focus:ring-4 focus:ring-navy-600/5 focus:border-navy-600/20' 
            ...
            <div className="flex items-center gap-3">
              {user ? (
                <Link 
                  ...
                  className="px-5 py-2 bg-navy-600 text-white rounded-full text-[11px] font-bold uppercase tracking-wider shadow-lg shadow-navy-600/20 hover:bg-navy-700 transition-all whitespace-nowrap"
                >
                  Dashboard
                </Link>
              ) : (
                <>
                  <Link 
                    to="/login" 
                    className={`hidden sm:block px-3 py-2 text-[11px] font-bold uppercase tracking-wider transition-all ${
                      showWhiteBg || isHome ? 'text-slate-600 hover:text-navy-600' : 'text-white/80 hover:text-white'
                    }`}
                  >
                    Log In
                  </Link>
                  <Link 
                    to="/signup" 
                    className="px-5 py-2.5 bg-slate-900 text-white rounded-full text-[11px] font-bold uppercase tracking-wider shadow-lg shadow-slate-900/20 hover:bg-slate-800 transition-all whitespace-nowrap"
                  >
                    Join Now
                  </Link>
                </>
            ...
            <div className="flex flex-col gap-6">
              ...
              {!user && (
                <div className="pt-4 border-t border-slate-100 flex flex-col gap-3">
                  ...
                  <Link to="/signup" className="w-full py-4 bg-navy-600 text-white text-center rounded-xl font-bold" onClick={() => setMobileMenuOpen(false)}>Create Account</Link>
                </div>
              )}
            </div>
          </div>
